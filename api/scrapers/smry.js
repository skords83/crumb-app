...
// FIX: NON_PHASE_H4 darf nur auf eigenständige Wörter/Phrasen matchen, nicht als
// Teilstring beliebig in der Überschrift vorkommen. Vorher führte z. B. "mehl" als
// Substring von "Mehlkochstück" dazu, dass diese (echte) Phase fälschlich verworfen
// wurde, bevor sie überhaupt gegen KNOWN_PHASES/PHASE_PATTERNS geprüft werden konnte.
// Jetzt matcht "mehl" nur noch, wenn es als eigenes Wort auftritt (z. B. Überschrift
// "Mehl" oder "Mehl & Wasser"), nicht mehr innerhalb von "Mehlkochstück".
function isNonPhaseHeading(h4Text) {
  const lower = h4Text.toLowerCase();
  return NON_PHASE_H4.some((s) => {
    const re = new RegExp(`(^|[^a-zäöüß])${escapeRegex(s)}([^a-zäöüß]|$)`, "i");
...
function detectStepType(text) {
  ...
  if (lower.match(/reifen|ruhen|gehen|aufgehen|stockgare|stückgare|gare\s/))
    return "Warten";
  if (
    lower.match(
      /\d+\s*(stunden?|minuten?|std|min|h)\s+(bei|reifen|ruhen|gehen)/i,
    )
  )
    return "Warten";
...
function parseRepeatingActions(instruction, totalDuration) {
  ...
  const matchA = instruction.match(
    /dabei\s+nach\s+([\d,.\sund]+)\s*(minuten?|stunden?)\s+(.+)/i,
  );
  if (matchA) {
  ...
  const matchB = instruction.match(
    /dabei\s+alle\s+(\d+)\s*minuten?\s+(.+?)(?:\s*\((\d+)x\))?\.?\s*$/i,
  );
  if (matchB) {
...
function extractIngredientsFromChunk(chunk) {
  ...
  while ((row = rowRe.exec(chunk)) !== null) {
    ...
    const hasAmount =
      /^\d/.test(firstCell) ||
      /^(gesamter?s?|nach\s+Belieben)/i.test(firstCell);
    ...
    const temperature = (hasAmount ? filteredCells[2] : filteredCells[1])
      ? (hasAmount ? filteredCells[2] : filteredCells[1])
          .replace("°C", "")
          .trim()
      : "";
...
function extractAllSteps(str) {
  ...
  const smryRe =
    /<div[^>]*>\s*<div>\s*<p>\s*(\d+)\s*<\/p>\s*<\/div>\s*<p>\s*([\s\S]*?)\s*<\/p>\s*<\/div>/gi;
  let smryM;
    ...
    while ((proseM = proseRe.exec(str)) !== null) {
      ...
      while ((pM = pRe.exec(proseM[1])) !== null) {
        ...
        if (
          ...
          !steps.some((s) => s.instruction === instruction)
        ) {
  ...
  while ((m = rgbaRe.exec(str)) !== null) {
    ...
    const displayMatches =
      before.match(/display:\s*(none|block|flex|grid)/g) || [];
...
const parseHtmlImport = async (html, filename, hostUrl) => {
  ...
  // BILD
  let imageUrl = "";
  const cloudimgMatch = html.match(
    /https?:\/\/[^"']*cloudimg\.io[^"']*\/entity\/gallery\/[^"']*\.jpg[^"']*/,
  );
  if (cloudimgMatch) {
    ...
    $("img").each((i, img) => {
      ...
      if (
        ...
        const size =
          (parseInt($(img).attr("width")) || 0) *
          (parseInt($(img).attr("height")) || 0);
    ...
    if (
      imageUrl &&
      (imageUrl.includes("_files/") || imageUrl.startsWith("Article%20"))
    ) {
      imageUrl = "";
      const ogUrl = $('meta[property="og:url"]').attr("content") || "";
      const ploetzMatch = ogUrl.match(
        /https?:\/\/(?:smry\.ai\/)?(.+ploetzblog\.de.+)/,
      );
      if (ploetzMatch) {
        const ploetzUrl = ploetzMatch[1].startsWith("http")
          ? ploetzMatch[1]
          : "https://" + ploetzMatch[1];
        ...
        try {
          ...
          if (
            ploetzOgImage &&
            ploetzOgImage.startsWith("http") &&
            !ploetzOgImage.includes(".svg")
          ) {
            imageUrl = ploetzOgImage
              .replace(/[?&]p=w\d+/g, "")
              .replace(/[?&]width=\d+/g, "");
          }
          const ploetzDesc =
  ...
  if (
    recipeData.original_source_url &&
    !recipeData.original_source_url.startsWith("http")
  ) {
    recipeData.original_source_url =
      "https://" + recipeData.original_source_url;
  }
  if (recipeData.original_source_url) {
    recipeData.original_source_url = recipeData.original_source_url.replace(
      /^http:\/\//i,
      "https://",
    );
  }
  ...
  $("table tr").each((i, tr) => {
    ...
    if (cells.length >= 2) {
      ...
      if (
        amount.match(/\d+[,.]?\d*\s*(g|kg|ml|l|%|EL|TL|Prise)/i) &&
        name &&
        name.length > 2
      ) {
  ...
  while ((h4Match = h4Re.exec(rawHtml)) !== null) {
    ...
    if (isNonPhaseHeading(h4Text)) continue;
    let found = false;
  ...
  if (uniquePhases.length === 0) {
    ...
    const expanded = [];
    allSteps.forEach((s) => {
      const rep = parseRepeatingActions(
        s.instruction,
        extractDuration(s.instruction) || 5,
      );
      if (rep) {
        expanded.push(...rep);
      } else {
        splitCompoundStep(s.instruction).forEach((step) => expanded.push(step));
      }
    });
    dough_sections = [
      {
        ...
        steps:
          expanded.length > 0
            ...
            : [
                {
                  instruction: "Alle Zutaten mischen",
                  duration: 10,
                  type: "Aktion",
                },
                {
                  instruction: "Teig ruhen lassen",
                  duration: 90,
                  type: "Warten",
                },
    ...
    for (let i = 0; i < uniquePhases.length; i++) {
      ...
      allSteps
        ...
        .forEach((s) => {
          ...
          if (rep) {
            expandedSteps.push(...rep);
          } else {
            splitCompoundStep(s.instruction).forEach((step) =>
  ...
  recipeData.steps = allSteps.map((s) => ({
    ...
    duration: extractDuration(s.instruction) || 5,
  ...
  const phaseDetails = dough_sections
    .map(
      (s) =>
        `${s.name} (${s.is_parallel ? "Parallel" : "Sequenziell"}, ${s.ingredients.length} Zutaten, ${s.steps.length} Schritte)`,
    )
    .join(" | ");
  const quantityMatches = recipeData.ingredients.filter((ing) =>
    /^(gesamter?s?|nach\s+Belieben)/i.test(ing.amount),
  ).length;
  console.log(`✅ ${dough_sections.length} Phasen: [${phaseDetails}]`);
  console.log(
    `   ${recipeData.steps.length} Schritte, ${recipeData.ingredients.length} Zutaten (${quantityMatches} mit gesamter/gesamtes/nach Belieben)`,
  );
