...
export default function BackplanPage() {
  ...
  // Auto-toggle: Zutaten aufklappen — bei mehreren parallelen Aktionen
  // nur die erste Phase aufklappen, damit die Seite nicht überwältigend wird.
  useEffect(() => {
    if (!session) return;
    const activeFocusPhases = sortedPhases.filter(
      (p) => !p.allDone && !p.allLocked && p.hasActive && (p.isSoftDone || p.isActiveAction),
    );
    const primaryPhaseName = activeFocusPhases[0]?.name ?? null;
    setOpenIngredients((prev) => {
      const n = new Set(prev);
      sortedPhases.forEach((p) => {
        const k = `${session.id}-${p.name}`;
        const isPrimary = p.name === primaryPhaseName;
        if (p.hasActive && p.ingredientsRelevant && isPrimary) {
          n.add(k);
        } else if (p.hasActive && (!p.ingredientsRelevant || !isPrimary)) {
          n.delete(k);
        }
      });
      return n;
      ...
      <div className="max-w-3xl mx-auto px-4 pt-4">
        ...
        {(gates.length > 0 || focusPhases.length > 0) && (
          <div className="mb-5">
            ...
            {/* Focus-Phase Cards:
                Bei 1 Phase → volle Card mit Zutaten + Timer.
                Bei mehreren gleichzeitigen Aktionen → kompakte Liste,
                nur erste Phase bekommt Zutaten aufgeklappt. */}
            {focusPhases.length === 1 ? (
              // ── EINZELNE FOCUS-PHASE: volle Card ──
              (() => {
                const phase = focusPhases[0];
                ...
                const IngredientsSection =
                  ings.length === 0 ? null : !phase.ingredientsRelevant ? (
                    ...
                    <div className="mb-2 rounded-xl bg-white/60 dark:bg-white/[0.03] border border-[#D6C9B4] dark:border-white/[0.07] overflow-hidden">
                      <button onClick={() => toggleIng(ik)} className="w-full flex items-center justify-between px-3 py-2 hover:bg-[#EDE5D6]/40 dark:hover:bg-white/[0.02] transition-colors">
                        <span className="text-[10px] font-extrabold uppercase tracking-wider text-[#8B7355]/80 dark:text-[#C4A484]/70">{phase.name} · Zutaten</span>
                        <div className="flex items-center gap-2">
                          {multiplier !== 1 && <span className="text-[10px] font-bold text-[#8B7355] dark:text-[#C4A484]">{multiplier}×</span>}
                  ...
                  return (
                    <div key={phase.name} className="mb-3 rounded-2xl border-2 border-amber-400/60 dark:border-amber-400/50 bg-amber-50 dark:bg-amber-500/[0.08] p-4 shadow-md shadow-amber-200/30 dark:shadow-amber-900/20">
                      ...
                      <div className="flex gap-2">
                        <button onClick={() => transition(session.id, activePhaseStep.globalIdx, "complete")} className="flex-1 py-3 rounded-xl bg-amber-500 hover:bg-amber-600 text-white font-extrabold text-[13px] active:scale-[0.98] shadow-sm">Ja, fertig</button>
                        <button onClick={() => transition(session.id, activePhaseStep.globalIdx, "extend_timer", { minutes: 15 })} className="px-3 py-3 rounded-xl bg-amber-100 dark:bg-amber-500/15 text-amber-800 dark:text-amber-300 font-bold text-[12px]">+15</button>
                        <button onClick={() => transition(session.id, activePhaseStep.globalIdx, "extend_timer", { minutes: 30 })} className="px-3 py-3 rounded-xl bg-amber-100 dark:bg-amber-500/15 text-amber-800 dark:text-amber-300 font-bold text-[12px]">+30</button>
                      </div>
                      {undoButton}
                    </div>
                  );
                ...
                if (isReadyBaking) {
                  const prevWasBaking = doneSteps.length > 0 && doneSteps[doneSteps.length - 1]?.type === "Backen";
                  return (
                    <React.Fragment key={phase.name}>
                      ...
                      <div className="mb-3 rounded-2xl border-2 border-dashed border-red-400/50 dark:border-red-400/40 bg-red-50/70 dark:bg-red-500/[0.06] p-4">
                        ...
                        <button onClick={() => transition(session.id, activePhaseStep.globalIdx, "start_baking")} className="w-full py-3 rounded-xl bg-red-500 hover:bg-red-600 text-white font-bold text-[13px] active:scale-[0.98]">
                          {prevWasBaking ? "Weiter backen" : "Ofen ist bereit — Backen starten"}
                        </button>
                        {undoButton}
                      </div>
                    </React.Fragment>
                ...
                const actionElapsed = activePhaseStep.start && ACTION_TYPES.has(activePhaseStep.type) ? Math.round((currentTime.getTime() - new Date(activePhaseStep.start).getTime()) / 1000) : null;
                const actionCountdownSec = (() => {
                  if (sRem !== null) return sRem;
                  if (!activePhaseStep.start || !ACTION_TYPES.has(activePhaseStep.type)) return null;
                  const durMin = activePhaseStep.duration_min; const durMax = activePhaseStep.duration_max;
                  if (durMin != null && durMax != null) return null;
                  const target = new Date(activePhaseStep.start).getTime() + activePhaseStep.duration * 60000;
                  return Math.max(0, Math.round((target - currentTime.getTime()) / 1000));
                })();
                const actionRangeStatus = (() => {
                  if (actionElapsed === null) return null;
                  const durMin = activePhaseStep.duration_min; const durMax = activePhaseStep.duration_max;
                  if (durMin == null || durMax == null) return null;
                  const e = actionElapsed / 60;
                  return e >= durMax ? "over" as const : e >= durMin ? "ready" as const : "running" as const;
                })();
                const cardStyle = isBaking ? "border-2 border-red-400/45 bg-red-50/50 dark:border-red-400/30 dark:bg-red-500/[0.06]" : "border-2 border-[#8B7355]/30 bg-[#8B7355]/[0.06] dark:border-[#C4A484]/30 dark:bg-[#C4A484]/[0.08]";
                const stepBadge = isBaking ? "bg-red-500 text-white" : "bg-[#8B7355]/15 text-[#8B7355] border border-[#8B7355]/20 dark:bg-[#C4A484]/20 dark:text-[#C4A484] dark:border-[#C4A484]/20";
                return (
                  <React.Fragment key={phase.name}>
                    {IngredientsSection}
                    <div className={`mb-3 rounded-2xl p-4 ${cardStyle}`}>
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="text-[10px] text-[#A68B6A] dark:text-white/30 font-bold">
                          {activePhaseStep.scheduled_start ? (new Date(activePhaseStep.scheduled_start) <= currentTime ? `seit ${formatSmartTime(new Date(activePhaseStep.scheduled_start))}` : `ab ${formatSmartTime(new Date(activePhaseStep.scheduled_start))}`) : ""}
                        </span>
                      </div>
                      <div className="flex items-start gap-2.5 px-3 py-2.5 rounded-xl mb-3 bg-white/40 dark:bg-white/[0.04]">
                        ...
                        {actionRangeStatus !== null && actionElapsed !== null ? (
                          <span className={`text-[13px] font-extrabold flex-shrink-0 tabular-nums ${actionRangeStatus === "over" ? "text-amber-600 dark:text-amber-400" : actionRangeStatus === "ready" ? "text-green-600 dark:text-green-400" : isBaking ? "text-red-600 dark:text-red-400" : "text-[#8B7355] dark:text-[#C4A484]"}`}>{formatCountdown(actionElapsed)}</span>
                        ) : actionCountdownSec !== null ? (
                          <span className={`text-[13px] font-extrabold flex-shrink-0 tabular-nums ${isBaking ? "text-red-600 dark:text-red-400" : "text-[#8B7355] dark:text-[#C4A484]"}`}>{actionCountdownSec > 0 ? formatCountdown(actionCountdownSec) : "0:00"}</span>
                        ) : (
                          <span className={`text-[11px] font-bold flex-shrink-0 tabular-nums ${isBaking ? "text-red-600 dark:text-red-400" : "text-[#8B7355] dark:text-[#C4A484]"}`}>{sRem !== null && sRem > 0 ? formatCountdown(sRem) : formatStepDuration(activePhaseStep)}</span>
                        )}
                      </div>
                      {actionRangeStatus !== null && actionElapsed !== null && (
                            ...
                            {(() => {
                              ...
                              return (
                                <div className="relative h-full">
                                  <div className={`absolute top-0 left-0 h-full rounded-full transition-all duration-1000 ${actionRangeStatus === "over" ? "bg-amber-500" : actionRangeStatus === "ready" ? "bg-green-500" : "bg-[#8B7355] dark:bg-[#C4A484]"}`} style={{ width: `${pct * 100}%` }} />
                                  <div className="absolute top-0 h-full w-0.5 bg-white/60 dark:bg-white/30" style={{ left: `${minPct * 100}%` }} />
                                </div>
                              );
                            })()}
                          </div>
                          <span className={`text-[10px] font-bold flex-shrink-0 ${actionRangeStatus === "over" ? "text-amber-600 dark:text-amber-400" : actionRangeStatus === "ready" ? "text-green-600 dark:text-green-400" : "text-[#A68B6A] dark:text-white/30"}`}>
                            {actionRangeStatus === "over" ? "Maximalzeit überschritten" : actionRangeStatus === "ready" ? "Fertig wenn bereit" : `Ziel: ${formatStepDuration(activePhaseStep)}`}
                          </span>
                        </div>
                      )}
                      {isBaking ? (
                        <button onClick={() => transition(session.id, activePhaseStep.globalIdx, "complete")} className="w-full py-3 rounded-xl bg-red-500 hover:bg-red-600 text-white font-bold text-[13px] active:scale-[0.98]">
                          {nextStepAfterActive?.type === "Backen" ? "Weiter" : "Raus aus dem Ofen"}
                        </button>
                      ) : (
                        <button onClick={() => transition(session.id, activePhaseStep.globalIdx, "complete")} className="w-full py-3 rounded-xl bg-[#8B7355] hover:bg-[#7A6347] text-white font-bold text-[13px] active:scale-[0.98]">Erledigt</button>
                      )}
                      {undoButton}
                    </div>
                  </React.Fragment>
              ...
              // ── MEHRERE GLEICHZEITIGE AKTIONEN: kompakte Liste ──
              // Zutaten der ersten Phase eingeblendet, restliche nur auf Tap
              <div className="flex flex-col gap-2">
                {focusPhases.map((phase, fIdx) => {
                  const sd = getSec(phase.name);
                  const ings = sd?.ingredients || [];
                  const ik = `${session.id}-${phase.name}`;
                  const isIO = openIngredients.has(ik);
                  ...
                  // Soft-done bekommt auch im Multi-Modus eine prominentere Darstellung
                  if (phase.isSoftDone) {
                    return (
                      <div key={phase.name} className="rounded-xl border border-amber-400/50 dark:border-amber-400/30 bg-amber-50/60 dark:bg-amber-500/[0.06] px-3 py-2.5 flex items-center gap-3">
                        <div className="flex-1 min-w-0">
                          <div className="text-[10px] font-extrabold uppercase tracking-wide text-amber-700 dark:text-amber-300 mb-0.5">{phase.name} · Bereit!</div>
                          <div className="text-[12px] text-[#2C1A0E] dark:text-white/80 truncate">{activePhaseStep.instruction}</div>
                        </div>
                        <button onClick={() => transition(session.id, activePhaseStep.globalIdx, "complete")} className="px-3 py-1.5 rounded-lg bg-amber-500 text-white font-bold text-[11px] flex-shrink-0">Fertig</button>
                      </div>
                    );
                  }

                  const ctaAction = isReadyBaking ? "start_baking" : "complete";
                  const ctaLabel = isReadyBaking ? "Backen starten" : isBaking ? "Raus" : "Erledigt";
                  const ctaColor = isReadyBaking || isBaking ? "bg-red-500 hover:bg-red-600" : "bg-[#8B7355] hover:bg-[#7A6347]";

                  return (
                    <div key={phase.name} className={`rounded-xl border ${isBaking || isReadyBaking ? "border-red-400/30 dark:border-red-400/20 bg-red-50/30 dark:bg-red-500/[0.04]" : "border-[#8B7355]/20 dark:border-[#C4A484]/15 bg-[#8B7355]/[0.04] dark:bg-[#C4A484]/[0.04]"}`}>
                      {/* Zutaten: erste Phase auto-auf, restliche kollabiert */}
                      {ings.length > 0 && (
                        <button onClick={() => toggleIng(ik)} className="w-full flex items-center justify-between px-3 pt-2.5 pb-1">
                          <span className="text-[10px] font-extrabold uppercase tracking-wide text-[#8B7355]/70 dark:text-[#C4A484]/60">{phase.name} · Zutaten</span>
                          <span className="text-[10px] text-[#8B7355]/50 dark:text-[#C4A484]/40 flex items-center gap-1">
                            {isIO ? <ChevronUp size={10} /> : <ChevronDown size={10} />}
                          </span>
                        </button>
                      )}
                      {ings.length > 0 && isIO && (
                        <div className="px-3 pb-1 grid grid-cols-2 gap-x-4">
                          {ings.map((ing: any, iIdx: number) => (
                            <div key={iIdx} className="flex justify-between text-[11px] py-0.5 border-b border-[#EDE5D6]/40 dark:border-white/[0.04] last:border-0">
                              <span className="text-[#5C3D1E]/70 dark:text-white/45">{ing.name}</span>
                              <span className="font-bold text-[#2C1A0E] dark:text-white/75">
                                {ing.amount ? `${scaleAmount(ing.amount, multiplier)} ${String(ing.amount || "").includes(ing.unit) ? "" : ing.unit || ""}` : ""}
                              </span>
                            </div>
                          ))}
                      ...
                      <div className="px-3 py-2.5 flex items-center gap-2.5">
                        <div className="flex-1 min-w-0">
                          <div className={`text-[10px] font-extrabold uppercase tracking-wide mb-0.5 ${isBaking || isReadyBaking ? "text-red-600 dark:text-red-400" : "text-[#8B7355] dark:text-[#C4A484]"}`}>
                            {ings.length === 0 ? phase.name : ""}
                          </div>
                          <div className="text-[12px] text-[#2C1A0E] dark:text-white/80 leading-snug">{activePhaseStep.instruction}</div>
                        </div>
                        <span className="text-[11px] font-bold text-[#A68B6A] dark:text-white/30 flex-shrink-0">{formatStepDuration(activePhaseStep)}</span>
                        <button onClick={() => transition(session.id, activePhaseStep.globalIdx, ctaAction)} className={`px-3 py-1.5 rounded-lg ${ctaColor} text-white font-bold text-[11px] flex-shrink-0 active:scale-[0.97] transition-all`}>{ctaLabel}</button>
                      </div>
                    </div>
                  );
                })}
