...
export default function BackplanPage() {
  ...
  const toggleDoneSteps = (key: string) =>
    setOpenDoneSteps((prev) => {
      const n = new Set(prev);
      n.has(key) ? n.delete(key) : n.add(key);
      return n;
    });
    ...
    <div className="min-h-screen bg-[#F5F0E8] dark:bg-[#0F172A] pb-32 transition-colors duration-200">
      ...
      <div className="max-w-3xl mx-auto px-4 pt-4">
        ...
        {gates.map((gate) => (
          <div
            key={gate.phase}
            className="mb-4 rounded-2xl border-2 border-dashed border-emerald-600/30 dark:border-emerald-500/30 bg-emerald-50 dark:bg-emerald-500/5 p-5"
          >
        ...
        {sortedPhases.map((phase, pIdx) => {
          ...
          const upcomingSteps = phase.steps.filter(
            (s) =>
              s.state !== "done" &&
              activePhaseStep &&
              s.globalIdx !== activePhaseStep.globalIdx,
          );
          // Nächster Step nach dem aktiven (für kontextabhängige Button-Texte)
          const nextStepAfterActive = activePhaseStep
            ? phase.steps.find(
                (s) =>
                  s.globalIdx > activePhaseStep.globalIdx && s.state !== "done",
              )
            : null;
          ...
          if (phase.allDone) {
            return (
              <div
                key={pIdx}
                className="mb-2 flex items-center gap-2.5 px-3 py-2 rounded-lg bg-transparent border-0 opacity-40"
              >
                <Check
                  size={12}
                  className="text-[#8B7355] dark:text-[#C4A484] flex-shrink-0"
                />
                <span className="text-[11px] font-semibold text-[#8B7355] dark:text-[#C4A484]/60 flex-1">
                  {phase.name}
                </span>
                <span className="text-[10px] text-[#C4A484] dark:text-white/20">
                  {phase.total} Schritte erledigt
                </span>
              </div>
            );
          }
          ...
          if (phase.allLocked) {
            ...
            return (
              <div
                key={pIdx}
                className="mb-2 flex items-center gap-2.5 px-3 py-2 rounded-lg bg-transparent border border-[#EDE5D6] dark:border-white/[0.04] opacity-55"
              >
                ...
                <span className="text-[11px] font-semibold text-[#A68B6A] dark:text-white/30 flex-1">
                  {phase.name}
                </span>
                <span className="text-[10px] text-[#C4A484] dark:text-white/20">
                  {startTime
                    ? `startet ~${formatSmartTime(startTime)}`
          ...
          if (!activePhaseStep) {
            ...
            return (
              <div
                key={pIdx}
                className="mb-2 flex items-center gap-2.5 px-3 py-2.5 rounded-xl bg-transparent border border-dashed border-[#D6C9B4] dark:border-white/[0.06] opacity-65"
              >
                <span className="w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-extrabold flex-shrink-0 bg-[#EDE5D6] text-[#C4A484] dark:bg-white/[0.06] dark:text-white/25">
                  {pIdx + 1}
                </span>
                <div className="flex-1 min-w-0">
                  <div className="text-[11px] font-semibold text-[#5C3D1E] dark:text-white/45 truncate">
                    {phase.name}
                  </div>
                  {nextStep && (
                    <div className="text-[10px] text-[#A68B6A] dark:text-white/30 truncate">
                      Pausiert · {phase.done}/{phase.total} erledigt · nächstes:{" "}
            ...
            <div className="mb-1">
              ...
              >
                ...
                <span className="text-[10px] text-[#8B7355] dark:text-[#C4A484] flex-1 text-left">
                  {doneSteps.length}{" "}
                  {doneSteps.length === 1 ? "Schritt" : "Schritte"} erledigt
                </span>
                {doneOpen ? (
                  <ChevronUp
                    size={11}
                    className="text-[#8B7355]/60 dark:text-[#C4A484]/60"
                  />
                ) : (
                  <ChevronDown
                    size={11}
              ...
              {doneOpen && (
                <div className="pl-1 flex flex-col gap-0.5 mt-0.5">
                  {doneSteps.map((step: TimelineStep) => (
                    <div
                      key={step.globalIdx}
                      className="flex items-center gap-2 py-0.5 opacity-30"
                    >
                      <Check
                        size={10}
                        className="text-[#8B7355] dark:text-[#C4A484] flex-shrink-0"
                      />
                      <span className="text-[11px] text-[#8B7355] dark:text-[#C4A484] line-through flex-1 truncate">
                        {step.instruction}
                      </span>
                      <span className="text-[10px] text-[#D6C9B4] dark:text-white/15 flex-shrink-0">
            ...
            <div className="mb-3 pl-1 flex flex-col gap-0.5">
              {upcomingSteps.slice(0, 6).map((step: TimelineStep) => (
                ...
                >
                  ...
                  <span
                    className={`inline-flex items-center px-1.5 py-0.5 rounded text-[9px] font-extrabold uppercase tracking-wide flex-shrink-0 ${
                      step.type === "Backen"
                        ? "bg-red-500/15 text-red-700 dark:text-red-400"
                        : WAIT_TYPES.has(step.type)
                  ...
                  <span className="text-[11px] text-[#A68B6A] dark:text-white/30 flex-1 truncate">
                    {step.instruction}
                  </span>
                  <span className="text-[10px] font-bold text-[#D6C9B4] dark:text-white/15 flex-shrink-0">
                    {formatStepDuration(step)}
                  </span>
                </div>
              ))}
              {upcomingSteps.length > 6 && (
                <span className="text-[10px] text-[#C4A484] dark:text-white/15 pl-4 pt-0.5">
                  + {upcomingSteps.length - 6} weitere
                </span>
              )}
            </div>
          );
          ...
          if (phase.isActiveWaiting) {
            ...
            const notice =
              urgency === "imminent" ? (
                <span className="inline-block text-[9px] font-extrabold uppercase tracking-wide px-2 py-0.5 rounded bg-amber-200 text-amber-800 dark:bg-amber-500/25 dark:text-amber-200">
                  Gleich geht's weiter
                </span>
              ) : urgency === "soon" ? (
                <span className="inline-block text-[9px] font-extrabold uppercase tracking-wide px-2 py-0.5 rounded bg-amber-100 text-amber-700 dark:bg-amber-500/15 dark:text-amber-300">
                  Bald weitermachen
                </span>
              ) : null;
                  ...
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    {waitRem !== null && waitRem > 0 && (
                      <span className={timerStyle}>
                        {formatCountdown(waitRem)}
                      </span>
                    )}
                  </div>
                  <div className="flex items-start gap-2 mb-2">
                    <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[9px] font-extrabold uppercase tracking-wide flex-shrink-0 mt-0.5 bg-amber-100 text-amber-700 dark:bg-amber-500/15 dark:text-amber-400">
                      {activePhaseStep.type}
                    </span>
                    <span
                      className={`text-[12px] flex-1 leading-snug ${urgency === "imminent" ? "text-[#2C1A0E] dark:text-white/85 font-semibold" : "text-[#5C3D1E] dark:text-white/70"}`}
                    >
                      {activePhaseStep.instruction}
                    </span>
                  </div>
          ...
          return (
            <React.Fragment key={pIdx}>
              ...
              <div className={`mb-2 rounded-2xl p-4 ${cardStyle}`}>
                <div className="flex items-center gap-2 mb-2">
                  <span
                    className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-extrabold flex-shrink-0 ${numBg}`}
                  >
                    {pIdx + 1}
                  </span>
                  <span className="text-[12px] font-extrabold flex-1 text-[#2C1A0E] dark:text-white/85 uppercase tracking-wide">
