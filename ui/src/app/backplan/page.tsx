...
function BackplanSkeleton() {
  return (<div className="min-h-screen bg-[#0F172A] flex items-center justify-center"><div className="animate-pulse flex flex-col items-center gap-4"><div className="w-16 h-16 rounded-full bg-white/10" /><div className="h-4 w-40 bg-white/10 rounded-full" /></div></div>);
}
...
export default function BackplanPage() {
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#0F172A] px-6">
      <div className="text-center">
        <div className="w-20 h-20 rounded-full bg-white/10 flex items-center justify-center mx-auto mb-6">
          <Sun size={32} className="text-[#C4A484]" />
        </div>
        <h2 className="text-2xl font-bold text-white/90 mb-2">Keine Backpläne aktiv</h2>
        <p className="text-white/40 mb-8">Plane ein Rezept um hier loszulegen.</p>
  ...
  return (
    <div className="min-h-screen bg-[#0F172A] pb-32 transition-colors duration-200">

      {/* ── Finish Modal ── */}
      {finishModalId !== null && (() => { const ms = sessions.find(s => s.id === finishModalId); if (!ms) return null; return (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center px-4 pb-6 sm:pb-0">
          <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={() => setFinishModalId(null)} />
          <div className="relative w-full max-w-sm bg-gray-900 rounded-3xl shadow-2xl border border-white/10">
            ...
            <div className="p-5">
              <p className="text-[13px] text-white/50 mb-3">Notizen zum Backergebnis (optional):</p>
              <textarea value={finishNotes} onChange={e => setFinishNotes(e.target.value)} placeholder="z.B. Krume perfekt, nächstes Mal länger Stockgare..." className="w-full h-20 px-3 py-2 rounded-xl border border-white/10 bg-white/5 text-sm text-white/80 resize-none focus:outline-none focus:ring-2 focus:ring-[#C4A484]/30 mb-4 placeholder:text-white/20" />
              ...
              <button onClick={() => setFinishModalId(null)} className="w-full py-2 text-white/30 font-bold text-[13px]">Weiterbacken</button>
            </div>
          </div>
        </div>); })()}
      ...
      {sessions.length > 1 && (
        <div className="bg-[#0F172A] border-b border-white/[0.07]">
          <div className="max-w-3xl mx-auto px-4 py-3">
            <p className="text-[10px] font-black uppercase tracking-widest text-white/20 mb-2">{sessions.length} aktive Backpläne</p>
            <div className="flex gap-2 overflow-x-auto pb-1">
              {sessions.map((s, idx) => { const p = getProgress(s.timeline || []); const isAct = idx === activeIdx; return (
                <button key={s.id} onClick={() => setActiveIdx(idx)} className={`flex-shrink-0 flex items-center gap-2.5 px-3 py-2 rounded-xl border transition-all ${isAct ? 'bg-[#C4A484]/20 border-[#C4A484]/40 text-white' : 'bg-white/5 border-white/10 text-white/60'}`}>
                  ...
                  <div className="text-left min-w-0">
                    <div className={`text-[12px] font-extrabold truncate max-w-[130px] ${isAct ? 'text-white' : 'text-white/70'}`}>{s.title}</div>
                    <div className={`text-[10px] font-bold ${isAct ? 'text-[#C4A484]' : 'text-white/30'}`}>{Math.round(p * 100)}%</div>
                  </div>
                </button>); })}
            </div>
          </div>
      ...
      {/* ── Sticky Header ── */}
      {/* ÄNDERUNG 1+2: Dunkel + DM Serif Titel */}
      <div className="sticky top-0 z-30 bg-[#0F172A] border-b border-white/[0.07]">
        <div className="max-w-3xl mx-auto px-4 pt-4 pb-3">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                {/* ÄNDERUNG 2: DM Serif Display für Rezepttitel */}
                <h1
                  className="text-[17px] leading-tight text-[#F5EDD8]"
                  style={{ fontFamily: 'var(--font-dm-serif), serif' }}
                >{session.title}</h1>
                <p className="text-[12px] text-[#C4A484]/70 font-medium flex items-center gap-1">
                  <Clock size={11} />
                  {projectedEnd ? `Fertig um ~${formatSmartTime(projectedEnd)} Uhr` : 'Berechne...'}
                </p>
              </div>
            </div>
            <button onClick={() => setFinishModalId(session.id)} className="px-3 py-2 rounded-xl bg-green-500/15 text-green-400 text-[11px] font-bold border border-green-500/20 hover:bg-green-500/25 transition-colors">
              Fertig
            </button>
          </div>

          {/* ÄNDERUNG 3: Pro-Teig-Fortschrittsbalken — unverändert, nur Farben angepasst */}
            ...
            {sortedPhases.map((phase) => {
              ...
              return (
                <div key={phase.name} className="flex items-center gap-2">
                  <span className={`text-[10px] w-[72px] flex-shrink-0 truncate font-bold ${isDone ? 'text-[#C4A484]/50' : isActive ? 'text-[#C4A484]' : 'text-white/20'}`}>
                    {phase.name}
                  </span>
                  <div className="flex-1 h-[3px] rounded-full bg-white/[0.07] overflow-hidden">
                    {isDone && <div className="h-full w-full rounded-full bg-[#C4A484]/40" />}
                    {isActive && <div className="h-full rounded-full bg-[#C4A484] animate-pulse" style={{ width: `${pct * 100}%` }} />}
                    {!isDone && !isActive && !isLocked && <div className="h-full rounded-full bg-[#C4A484]/25" style={{ width: `${pct * 100}%` }} />}
                  </div>
                  <span className={`text-[10px] w-5 text-right flex-shrink-0 font-bold ${isDone ? 'text-[#C4A484]/50' : isActive ? 'text-[#C4A484]' : 'text-white/20'}`}>
      ...
      <div className="max-w-3xl mx-auto px-4 pt-4">
        ...
        {/* Gate-Cards */}
        {gates.map(gate => (
          <div key={gate.phase} className="mb-4 rounded-2xl border-2 border-dashed border-emerald-500/30 bg-emerald-500/5 p-5">
            <div className="text-[13px] font-extrabold text-emerald-400 mb-1">{gate.phase} kann starten</div>
            <p className="text-[12px] text-white/40 mb-3">Alle Vorstufen sind fertig.</p>
            <div className="flex gap-2 mb-3 flex-wrap">
              {gate.dependencies.map(dep => (<span key={dep} className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-emerald-500/10 text-emerald-400 text-[11px] font-bold"><Check size={10} /> {dep}</span>))}
            </div>
        ...
        {allSoftDoneSteps.map((sdStep) => (
          <div key={sdStep.globalIdx} className="mb-4 rounded-2xl border-2 border-amber-400/30 bg-amber-500/5 p-5">
            <div className="flex justify-between items-center mb-2">
              <span className="text-[10px] font-extrabold uppercase tracking-widest text-amber-400">{sdStep.phase}</span>
              <span className="text-[11px] font-bold text-amber-400">Zeit abgelaufen</span>
            </div>
            <p className="text-[14px] font-semibold text-white/90 mb-4 leading-relaxed">{sdStep.instruction}</p>
            <div className="flex gap-2">
              ...
              <button onClick={() => transition(session.id, sdStep.globalIdx, 'extend_timer', { minutes: 15 })} className="px-4 py-3 rounded-xl bg-white/10 text-white/60 font-bold text-[13px]">+15 Min</button>
              <button onClick={() => transition(session.id, sdStep.globalIdx, 'extend_timer', { minutes: 30 })} className="px-4 py-3 rounded-xl bg-white/10 text-white/60 font-bold text-[13px]">+30 Min</button>
            </div>
          </div>
        ))}

        {/* ÄNDERUNG 4: Primäre aktive Hero-Card — dunkler Brand-Ton */}
        {activeStep && allSoftDoneSteps.length === 0 && (
          <div ref={activeCardRef} className="mb-4 rounded-2xl border-2 border-[#C4A484]/30 bg-[#C4A484]/[0.08] p-5">
            <div className="flex justify-between items-center mb-2">
              <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-lg text-[10px] font-extrabold uppercase tracking-wide ${activeStep.type === 'Backen' ? 'bg-red-500 text-white' : activeStep.type === 'Aktion' || activeStep.type === 'Kneten' ? 'bg-[#C4A484]/30 text-[#C4A484] border border-[#C4A484]/30' : 'bg-white/10 text-white/60'}`}>{activeStep.phase}</span>
              <span className="text-[11px] text-white/30 font-bold">{activeStep.start ? `seit ${formatSmartTime(new Date(activeStep.start))}` : ''}</span>
            </div>
            <p className="text-[14px] font-semibold text-white/90 mb-3 leading-relaxed">{activeStep.instruction}</p>
            {remaining !== null && remaining > 0 && activeStep.type !== 'Aktion' && activeStep.type !== 'Kneten' && (
              <div className="flex items-center gap-3 mb-3 p-3 rounded-xl bg-white/5 border border-white/10">
                <div className="w-11 h-11 rounded-full border-[3px] border-white/10 flex items-center justify-center relative">
                  <svg className="absolute inset-0 w-full h-full -rotate-90" viewBox="0 0 44 44">
                    <circle cx="22" cy="22" r="19" fill="none" stroke="#C4A484" strokeWidth="3" strokeDasharray={`${timerProgress * 119.4} 119.4`} strokeLinecap="round" />
                  </svg>
                  <span className="text-[9px] font-bold text-[#C4A484]">{Math.round(timerProgress * 100)}%</span>
                </div>
                <div>
                  <div className="text-[9px] font-bold text-white/30 uppercase tracking-widest">Verbleibend</div>
                  <div className="text-[20px] font-extrabold text-white/90 tabular-nums">{formatCountdown(remaining)}</div>
                </div>
                {activeStep.extended_by > 0 && <span className="ml-auto text-[10px] font-bold text-amber-400 bg-amber-400/10 px-2 py-1 rounded-lg">+{activeStep.extended_by} Min</span>}
              </div>
            )}
            <div className="flex gap-2">
              {activeStep.type === 'Backen' ? (
                ...
                <>
                  ...
                  <button onClick={() => transition(session.id, activeStep.globalIdx, 'extend_timer', { minutes: 15 })} className="px-4 py-3 rounded-xl bg-white/10 text-white/60 font-bold text-[12px]">+15 Min</button>
                </>
              ) : (
                <button onClick={() => transition(session.id, activeStep.globalIdx, 'complete')} className="flex-1 py-3 rounded-xl bg-[#C4A484] hover:bg-[#b8956e] text-[#1C1410] font-bold text-[13px] active:scale-[0.98]">Erledigt</button>
              )}
            </div>
          </div>
        )}
        ...
        {parallelActiveSteps.length > 0 && allSoftDoneSteps.length === 0 && parallelActiveSteps.map(pStep => {
          ...
          return (
            <div key={pStep.globalIdx} className="mb-4 rounded-2xl border-2 border-dashed border-white/10 bg-white/[0.03] p-4">
              <div className="flex justify-between items-center mb-2">
                <div className="flex items-center gap-2">
                  <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-lg text-[9px] font-extrabold uppercase tracking-wide bg-white/10 text-white/40">Parallel</span>
                  <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-lg text-[9px] font-extrabold uppercase tracking-wide ${pStep.type === 'Backen' ? 'bg-red-500 text-white' : pStep.type === 'Aktion' || pStep.type === 'Kneten' ? 'bg-[#C4A484]/20 text-[#C4A484]' : 'bg-white/10 text-white/40'}`}>{pStep.phase}</span>
                </div>
                {pRemaining !== null && pRemaining > 0 && <span className="text-[12px] font-bold text-[#C4A484] tabular-nums">{formatCountdown(pRemaining)}</span>}
              </div>
              <p className="text-[13px] text-white/70 mb-3 leading-relaxed">{pStep.instruction}</p>
              <div className="flex gap-2">
                {pStep.type === 'Warten' || pStep.type === 'Ruhen' || pStep.type === 'Kühl' ? (
                  <>
                    ...
                    <button onClick={() => transition(session.id, pStep.globalIdx, 'extend_timer', { minutes: 15 })} className="px-3 py-2.5 rounded-xl bg-white/10 text-white/50 font-bold text-[11px]">+15 Min</button>
                  </>
                ) : (
        ...
        {timeline.filter(s => s.state === 'ready' && s.type === 'Backen').map(step => (
          <div key={step.globalIdx} className="mb-4 rounded-2xl border-2 border-dashed border-red-400/30 bg-red-500/5 p-5">
            <span className="text-[10px] font-extrabold uppercase text-red-400 mb-2 block">Bereit zum Backen</span>
            <p className="text-[13px] text-white/80 mb-3">{step.instruction}</p>
        ...
        {!focusStep && gates.length === 0 && timeline.length > 0 && timeline.every(s => s.state === 'done') && (
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-bold text-white/90 mb-2">Alle Schritte erledigt!</h3>
            <p className="text-white/40 mb-6">Du kannst den Backplan jetzt abschließen.</p>
        ...
        {/* ÄNDERUNG 5: Phasen mit Schritt-Liste */}
          ...
          return (
            <div key={pIdx} className="mb-6">
              ...
              <div className="flex items-center gap-3 mb-2 px-1">
                <span className={`w-7 h-7 rounded-full flex items-center justify-center text-[11px] font-extrabold flex-shrink-0 ${phase.hasActive ? 'bg-[#C4A484]/30 text-[#C4A484] border border-[#C4A484]/40' : phase.allDone ? 'bg-green-500/15 text-green-400' : 'bg-white/[0.06] text-white/25'}`}>
                  {phase.allDone ? <Check size={13} /> : pIdx + 1}
                </span>
                <span className={`text-[13px] font-extrabold flex-1 ${phase.allLocked ? 'text-white/20' : phase.allDone ? 'text-white/40' : 'text-white/80'}`}>{phase.name}</span>
                <span className="text-[11px] text-white/25 font-bold">{phase.done}/{phase.total}</span>
                {ings.length > 0 && (
                  <button onClick={() => toggleIng(ik)} className="flex items-center gap-1 text-[10px] font-bold text-[#C4A484]/70 hover:text-[#C4A484] transition-colors">
              ...
              {/* Zutaten-Drawer */}
              {isIO && ings.length > 0 && (
                <div className="ml-10 mb-3 bg-white/[0.04] border border-white/[0.07] rounded-xl p-3">
                  {ings.map((ing: any, iIdx: number) => (
                    <div key={iIdx} className="flex justify-between text-[11px] py-1.5 border-b border-white/[0.06] last:border-0">
                      <span className="text-white/50">{ing.name}</span>
                      <span className="font-bold text-white/80">{ing.amount ? `${scaleAmount(ing.amount, multiplier)} ${String(ing.amount||'').includes(ing.unit) ? '' : ing.unit||''}` : ''}</span>
                    </div>
                  ))}
                </div>
              )}

              {/* ÄNDERUNG 5: Erledigte Schritte inline als kompakte Dots */}
                ...
                <div className="pl-10 mb-1.5">
                  {doneS.map((step: TimelineStep) => (
                    <div key={step.globalIdx} className="flex items-center gap-2 py-0.5">
                      <div className="w-1.5 h-1.5 rounded-full bg-[#C4A484]/40 flex-shrink-0" />
                      <span className="text-[11px] text-white/20 line-through flex-1 truncate">{step.instruction}</span>
                      <span className="text-[10px] text-white/15 flex-shrink-0">{formatStepDuration(step)}</span>
                    </div>
                  ))}
                </div>
              )}
                ...
                {pendS.map((step: TimelineStep) => {
                  ...
                  return (
                    <div key={step.globalIdx} className={`flex items-start gap-2.5 px-3 py-2.5 rounded-xl transition-all ${
                      isA ? 'border-2 border-[#C4A484]/30 bg-[#C4A484]/[0.08]'
                      : isR ? 'border border-dashed border-white/15 bg-white/[0.03]'
                      : isL ? 'border border-white/[0.04] bg-transparent opacity-30'
                      : 'border border-white/[0.06] bg-white/[0.02]'
                    }`}>
                      <div className={`w-2 h-2 rounded-full flex-shrink-0 mt-1.5 ${isA ? 'bg-[#C4A484] animate-pulse' : isR ? 'bg-amber-400' : 'bg-white/15'}`} />
                      <span className={`inline-flex items-center px-2 py-0.5 rounded-lg text-[9px] font-extrabold uppercase tracking-wide flex-shrink-0 mt-0.5 ${
                        ...
                        : step.type === 'Aktion' || step.type === 'Kneten' ? 'bg-[#C4A484]/20 text-[#C4A484] border border-[#C4A484]/20'
                        : 'bg-white/[0.06] text-white/30'
                      } ${isL ? 'opacity-50' : ''}`}>{step.type}</span>
                      <span className={`text-[12px] flex-1 leading-snug ${isL ? 'text-white/20' : isA ? 'text-white/90' : 'text-white/55'}`}>{step.instruction}</span>
                      <span className={`text-[11px] font-bold flex-shrink-0 ${isA && sRem ? 'text-[#C4A484]' : 'text-white/20'}`}>{isA && sRem ? formatCountdown(sRem) : formatStepDuration(step)}</span>
                    </div>
                  );
                })}
              </div>
        ...
        {multiplier !== 1 && (
          <div className="mt-2 text-center">
            <span className="text-[10px] font-bold text-[#C4A484] bg-[#C4A484]/10 px-3 py-1 rounded-lg">{multiplier}x Menge</span>
          </div>
        )}
        <div className="mt-6">
          <Link href={`/recipes/${session.recipe_id}`} className="block w-full text-center py-3 rounded-2xl bg-white/[0.04] text-white/40 font-bold text-[12px] border border-white/[0.07] hover:bg-white/[0.07] transition-colors">Ganzes Rezept zeigen</Link>
        </div>
      </div>
    </div>
  );
